      ...
      <Dialog as='form' onClose={clearModalClose} className='flex absolute top-60 m-auto left-0 right-0'>
        <div className="fixed inset-0 bg-black/30 bg-opacity-25" />
          <Dialog.Panel className={`${isDark ? 'bg-[#1F2022]' : 'bg-slate-300'} transition-all mx-auto max-w-md transform overflow-hidden p-6 rounded-2xl shadow-xl align-middle md:w-full w-96`}>
            <Dialog.Title className={`${isDark ? 'text-white' : 'text-black'} text-center mb-3 text-[18px]`}>Are you sure you want to clear all {title} tasks? </Dialog.Title>
            <Dialog.Title className={`${isDark ? 'text-gray-400' : 'text-gray-500'} text-start md:ml-[17px] md:mb-9 mb-3 text-[15px]`}>This event cannot be undone. </Dialog.Title>
            <div>
            </div>
