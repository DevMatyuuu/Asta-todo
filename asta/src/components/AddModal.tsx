    ...
    <Transition show={isAddModalOpen} as={Fragment}>
      <Dialog as='form' onSubmit={handleAddTask} onClose={addModalClose} className='flex absolute top-60 m-auto left-0 right-0 '>
        <div className="fixed inset-0 bg-black/30 bg-opacity-25" />
          <Dialog.Panel data-aos='fade-in' className={`${isDark ? 'bg-[#1F2022]' : 'bg-slate-300'} transition-all mx-auto max-w-md transform overflow-hidden p-6 rounded-2xl shadow-xl align-middle md:w-full w-96`}>
